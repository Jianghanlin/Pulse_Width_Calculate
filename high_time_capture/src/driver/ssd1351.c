...
void Display_Chinese(unsigned char x, unsigned char y, unsigned char Var_Char, int Colour_RGB)
{
  ...
  {
    Var_Temp = my_name[Var_Char][i]; //获取汉字在数组的偏移量
    for(j = 0; j < 8; j++)
    {
      if(Var_Temp & 0x80)   //先画最高位的点，为1则画对应的颜色
          ssd1351_draw_point(x, y, Colour_RGB);
      //else
...
void dislay_topic()
{
    Display_Chinese(0,0,0,White);
    Display_Chinese(15,0,1,White);
    Display_Chinese(30,0,2,White);
    Display_Chinese(45,0,3,White);
    ssd1351_display_char(60, 0,'&', 1, White, Black);
    Display_Chinese(68,0,4,White);
    Display_Chinese(83,0,5,White);
    Display_Chinese(98,0,6,White);
    Display_Chinese(113,0,7,White);
}
